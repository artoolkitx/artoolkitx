...
if [ "$OS" = "Windows" ] ; then
    ...
    if [ $PACKAGE_WINDOWS ] ; then
        ...
        ${OURDIR}/windows/tools/rsync.exe -ar --files-from=${OURDIR}/windows/bom --exclude-from=${OURDIR}/windows/excludes ${ARTOOLKITX_HOME} ${TARGET_DIR}

        #Package all into a zip file
        cd ./windows/package/
        ${OURDIR}/windows/tools/zip.exe --filesync -r "artoolkitx-${VERSION}${VERSION_DEV+-dev}-Windows.zip" ./artoolkitX/
